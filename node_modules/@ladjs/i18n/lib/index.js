...
class I18N {
  constructor(config = {}) {
    ...
    Object.assign(this, i18n); // expose shorthand API methods
  ...
  middleware(ctx, next) {
    ...
    if (!locale) {
      ...
      } else if (this.config.lastLocaleField && isFunction(ctx.isAuthenticated) && ctx.isAuthenticated() && ctx.state.user[this.config.lastLocaleField]) {
  ...
  redirect(ctx, next) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this.config.lastLocaleField && isFunction(ctx.isAuthenticated) && ctx.isAuthenticated() && ctx.state.user[_this.config.lastLocaleField] !== locale) {
